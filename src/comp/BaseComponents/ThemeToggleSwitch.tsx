...
function ThemeToggleSwitch() {
    ...
    useEffect(() => {
        if (cookies.isDark == 'true')
        {
            document.documentElement.classList.add('dark');
        }
        else if (cookies.isDark == 'false') {
    ...
    function setTheme() {
        if (cookies.isDark == 'true')
        {
            setCookie('isDark', false, { path: '/' });
            document.documentElement.classList.remove('dark');
        }
        else if (cookies.isDark == 'false') {
            setCookie('isDark', true, { path: '/' });
            document.documentElement.classList.add('dark');
        }
    }

        return (
        <label htmlFor="default-toggle" className="inline-flex relative items-end cursor-pointer">
                <input type="checkbox" value="" id="default-toggle" className="sr-only peer" defaultChecked={cookies.isDark == 'true'}
                       onClick={() => setTheme()} onLoad={() => setTheme()}/>
                <div
